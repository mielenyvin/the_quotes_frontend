...
<div id="newsarticle" className='cursor-pointer h-full w-full bg-qverylitegray rounded-lg shadow-md hover:shadow-lg' >
    ...
    <span className='ml-4 text-metal text-sm font-bold'>{nw.title1}
     </span>
    </div> 
    </div>     

    <divider className='border-metal'><div className='px-2 pt-0 leading-5 pb-1  text-black rounded-xl'>Vs</div></divider>

    <div className='mx-5 mt-2 mb-6' >
    <div className='flex items-center'>
        {nw.image && (<img className='border p-1' src={nw.media_2_logo.url} width={60} height={60} /> ) }  
        <span className='ml-4 text-metal text-sm font-bold'>
    {nw.title2}</span>
    </div>  
    </div>              
